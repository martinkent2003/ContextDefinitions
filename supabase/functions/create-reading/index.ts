...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof title !== 'string' || title.trim().length === 0) {
      return jsonResponse(
        { ok: false, error: 'Invalid payload: non-empty "title" required' },
        400,
      )
    }
    if (typeof genre !== 'string' || genre.trim().length === 0) {
      return jsonResponse(
        { ok: false, error: 'Invalid payload: non-empty "genre" required' },
        400,
      )
    }
    ...
    if (typeof content !== 'string' || content.trim().length === 0) {
      return jsonResponse(
        { ok: false, error: 'Invalid payload: non-empty "content" required' },
        400,
      )
    }
